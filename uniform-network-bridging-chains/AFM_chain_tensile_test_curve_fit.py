...
characterization module for cuFJCs that undergo scission
"""
...
class AFMChainTensileTestCurveFitCharacterizer(Characterizer):
    """The characterization class assessing the single-chain AFM tensile
    test curve fit validation for cuFJCs that undergo scission. It
    inherits all attributes and methods from the ``Characterizer``
    class.
    """
    ...
    def characterization(self):
        ...
        # nm, nN respectively
        r_nu, f_c = (
            chain_data_file_load_func(data_file_prefix, cp.chain_data_directory)
        )
        ...
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-l_b_eq'+'.txt',
            [l_b_eq])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-l_nu_eq'+'.txt',
            [l_nu_eq])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-l_nu_eq_intgr_nu'+'.txt',
            [l_nu_eq_intgr_nu])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-nu_b'+'.txt',
            [nu_b])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-nu_b_intgr_nu'+'.txt',
            [nu_b_intgr_nu])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-nu'+'.txt',
            [nu])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-intgr_nu'+'.txt',
            [intgr_nu])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-zeta_b_char'+'.txt',
            [zeta_b_char])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-zeta_nu_char'+'.txt',
            [zeta_nu_char])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-zeta_nu_char_intgr_nu'+'.txt',
            [zeta_nu_char_intgr_nu])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-kappa_b'+'.txt',
            [kappa_b])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-kappa_b_intgr_nu'+'.txt',
            [kappa_b_intgr_nu])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-kappa_nu'+'.txt',
            [kappa_nu])
        np.savetxt(
            self.savedir+data_file_prefix+'-cuFJC-curve-fit-kappa_nu_intgr_nu'+'.txt',
            [kappa_nu_intgr_nu])

        # Initialize a RateDependentScissioncuFJC which matches the
        # chain used in the AFM tensile test where the bond and
        # segment numbers in the chain are strictly integer values
        ...
        # Initialize a RateIndependentScissioncuFJC which matches
        # the chain used in the AFM tensile test where the bond and
        # segment numbers in the chain are strictly integer values
    ...
    def finalization(self):
        ...
        fig = plt.figure()
        plt.scatter(
            lmbda_c_eq, xi_c, color='blue', marker='o', alpha=1, linewidth=2.5,
            label=cp.paper_authors2polymer_type_label_dict[self.paper_authors])
        plt.plot(
            lmbda_c_eq_fit, xi_c_fit, linestyle='--', color='red', alpha=1,
            linewidth=2.5, label=r'$\textrm{c}u\textrm{FJC model fit}$')
        plt.legend(loc='best', fontsize=18)
        plt.grid(True, alpha=0.25)
        plt.xlim([0, 1.15])
        ...
        # statistical chain scission probability and energy dissipation
        dashed_line_num = 1000
        dashed_line_x_axis = np.linspace(
            rate_independent_lmbda_c_eq[0], rate_independent_lmbda_c_eq[-1]+0.035,
            dashed_line_num)
        rate_independent_overline_epsilon_cnu_diss_hat_crit = (
            rate_independent_overline_epsilon_cnu_diss_hat[-1]
        )
        dashed_line_y_axis = (
            rate_independent_overline_epsilon_cnu_diss_hat_crit
            * np.ones(dashed_line_num)
        )
        fig, (ax1, ax2) = plt.subplots(
            2, 1, gridspec_kw={'height_ratios': [1, 1]}, sharex=True)
        ax1.plot(
            ...
            linestyle='-', color='blue', alpha=1, linewidth=2.5)
        ax1.plot(
            rate_independent_lmbda_c_eq[-1], rate_independent_p_c_sci_hat[-1],
            marker='x', color='blue', alpha=1, linewidth=2.5)
        ax2.plot(
            rate_independent_lmbda_c_eq,
            rate_independent_overline_epsilon_cnu_diss_hat, linestyle='-',
            color='blue', alpha=1, linewidth=2.5)
        ax2.plot(
            rate_independent_lmbda_c_eq[-1],
            rate_independent_overline_epsilon_cnu_diss_hat_crit, marker='x',
            color='blue', alpha=1, linewidth=2.5)
        ax2.plot(
            dashed_line_x_axis, dashed_line_y_axis,
            linestyle='--', color='black', alpha=1, linewidth=1)
        ax2.text(
            rate_independent_lmbda_c_eq[-1]+0.05,
            rate_independent_overline_epsilon_cnu_diss_hat_crit,
            r'$\hat{\mathcal{E}}_c^{\star} = \left(\hat{\mathcal{E}}_c^{diss}\right)^{crit}$',
            fontsize=20, horizontalalignment='left', verticalalignment='center')
        ax1.set_yticks([0, 1])
        ax1.set_ylim([-0.05, 1.05])
        ax1.tick_params(axis='y', labelsize=20)
        ax1.set_ylabel(r'$\hat{p}_c^{sci}$', fontsize=24)
        ax1.grid(True, alpha=0.25)
        ax2.set_yticks([0])
        ax2.set_ylim([-0.025, 0.425])
        ax2.tick_params(axis='y', labelsize=20)
        ax2.set_ylabel(r'$\hat{\mathcal{E}}_c^{diss}$', fontsize=24)
        ax2.grid(True, alpha=0.25)
        plt.xlim([1, 2])
        plt.xticks([])
        plt.xlabel(r'$f_c, r_{\nu}$', fontsize=30)
        save_current_figure_no_labels(
            self.savedir,
            data_file_prefix+"-statistical-chain-scission-probability-energy-dissipation")
        
        # discrete chain scission probability and energy dissipation
        dashed_line_num = 1000
        dashed_line_x_axis = np.linspace(
            rate_independent_lmbda_c_eq[0], rate_independent_lmbda_c_eq[-1]+0.035,
            dashed_line_num)
        ...
        data_num = 1000
        data_x_axis = np.linspace(
            rate_independent_lmbda_c_eq[0], rate_independent_lmbda_c_eq[-1],
            data_num)
        data_y_axis = np.zeros(data_num)
        fig, (ax1, ax2) = plt.subplots(
            2, 1, gridspec_kw={'height_ratios': [1, 1]}, sharex=True)
        ax1.plot(
            data_x_axis, data_y_axis,
            linestyle='-', color='blue', alpha=1, linewidth=2.5)
        ax1.plot(
        ...
        ax1.plot(
            data_x_axis[-1], 1,
            marker='.', color='blue', alpha=1, linewidth=2.5)
        ax1.plot(
            data_x_axis[-1], 1,
            marker='x', color='blue', alpha=1, linewidth=2.5)
        ax2.plot(
        ...
        ax2.plot(
            ...
            alpha=1, linewidth=1)
        ax2.plot(
            data_x_axis[-1], rate_independent_overline_epsilon_cnu_diss_hat_crit,
            marker='.', color='blue', alpha=1, linewidth=2.5)
        ax2.plot(
            data_x_axis[-1], rate_independent_overline_epsilon_cnu_diss_hat_crit,
            marker='x', color='blue', alpha=1, linewidth=2.5)
        ...
        ax2.text(
            rate_independent_lmbda_c_eq[-1]+0.05,
            rate_independent_overline_epsilon_cnu_diss_hat_crit,
            r'$\hat{\mathcal{E}}_c^{\star} = \left(\hat{\mathcal{E}}_c^{sci}\right)^{crit}$',
            fontsize=20, horizontalalignment='left', verticalalignment='center')
        ax1.set_yticks([0, 1])
        ax1.set_ylim([-0.05, 1.05])
        ax1.tick_params(axis='y', labelsize=20)
        ...
        ax2.set_ylim([-0.025, 0.425])
        ax2.tick_params(axis='y', labelsize=20)
        ax2.set_ylabel(r'$\hat{\mathcal{E}}_c^{sci}$', fontsize=24)
        ax2.grid(True, alpha=0.25)
        plt.xlim([1, 2])
        plt.xticks([])
        plt.xlabel(r'$f_c, r_{\nu}$', fontsize=30)
        save_current_figure_no_labels(
            self.savedir,
            data_file_prefix+"-discrete-chain-scission-probability-energy-dissipation")
...
if __name__ == '__main__':
    ...
    al_maawali_et_al_AFM_chain_tensile_test_fit_characterizer = (
        AFMChainTensileTestCurveFitCharacterizer(
            paper_authors="al-maawali-et-al", chain="chain-a", T=T)
    )
    # al_maawali_et_al_AFM_chain_tensile_test_fit_characterizer.characterization()
    al_maawali_et_al_AFM_chain_tensile_test_fit_characterizer.finalization()

    del al_maawali_et_al_AFM_chain_tensile_test_fit_characterizer

    hugel_et_al_AFM_chain_tensile_test_fit_characterizer = (
        AFMChainTensileTestCurveFitCharacterizer(
            paper_authors="hugel-et-al", chain="chain-a", T=T)
    )
    # hugel_et_al_AFM_chain_tensile_test_fit_characterizer.characterization()
    hugel_et_al_AFM_chain_tensile_test_fit_characterizer.finalization()

    del hugel_et_al_AFM_chain_tensile_test_fit_characterizer